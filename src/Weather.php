...
class Weather extends AbstractMessageHandler
{
    ...
    public function queryWeaher($name){
        if (in_array($name,$this->getCityData())){
            $new = array_flip($this->getCityData());
            $result = json_decode(file_get_contents('http://t.weather.sojson.com/api/weather/city/'.$new[$name]),true);
            $arr = [];
